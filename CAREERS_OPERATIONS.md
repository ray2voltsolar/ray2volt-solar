...
- Form: `hiring-form`. Optional fields: date of birth, email and previous work responsibilities. No Resume field is requested. No role-specific screening questions.
...
- Technical verification on 8 September 2026: Web3Forms accepted the clearly labeled no-attachment test with HTTP 200 and `success: true`. The owner then confirmed receipt of `[TEST - IGNORE] Ray2Volt hiring form email notification` in the usual website enquiries inbox. Email notification delivery is therefore confirmed by provider acceptance and owner receipt.
- The current Web3Forms plan rejects file uploads. The owner chose to leave Resume out for now; the previously approved link field and its URL validation have been removed. No Resume link or file is collected.
- This is a static GitHub Pages site. Required-field, phone, date and selection validation run in the browser; Web3Forms handles the server-side mail transport and anti-spam. There is no custom backend enforcing the recruitment schema, so treat all received applicant data as untrusted and verify in screening.
...
- Six focused checks run with `node --test tests/careers.test.cjs`. JavaScript syntax can be checked with `node --check careers.js`.
...
## Field simplification — 8 September 2026

- Previous work responsibilities is optional; applicants without experience can leave it blank.
- Resume is omitted per owner decision. The link control and its validation were removed together. No nonfunctional file upload is presented.
- The email service, notification route and successful-submission event remain unchanged. Six focused submission tests pass; no new email test was sent.
